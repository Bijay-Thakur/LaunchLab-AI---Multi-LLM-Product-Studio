"""Deterministic mock outputs for LaunchLab AI demo.

These mimic responses we would expect from a multi-LLM pipeline:
Gemini -> ChatGPT (architect + prompt eng + campaign) -> Claude Code -> GPT Image -> Gemini judge.
"""

PRODUCT_NAME = "BridgeMate"
SAMPLE_IDEA = "An app for international students who feel lonely after moving to the U.S."


def build_research(idea: str) -> dict:
    return {
        "summary": (
            "International students transitioning to U.S. universities face a sharp social and emotional adjustment "
            "in their first 6-12 months. Existing tools focus on logistics (housing, visas) but underserve the deeper "
            "loneliness, cultural disorientation, and mental-health gap."
        ),
        "problemValidation": [
        ...
        "marketGap": (
            "There is no warm, low-pressure, culturally-aware product that bridges social belonging and "
            "lightweight mental wellness for international students. The opportunity sits between friendship apps "
            "and mental-health apps - a 'soft landing' product."
        ),
        "ethicalConcerns": [
        ...
        "recommendedMvp": (
            "Build a small, opinionated app that pairs new international students with a 'BridgeMate' - a slightly "
            "more senior international student at the same university - plus a weekly themed group hangout and a "
            "private journaling space with optional gentle reflections. Skip therapy features in v1."
        ),
    }


def build_blueprint(idea: str) -> dict:
    return {
        "productName": PRODUCT_NAME,
        "onelinePitch": (
            "BridgeMate is a soft landing app that pairs new international students with a senior peer mentor and "
            "a small weekly group, so the first semester abroad feels less lonely."
        ),
        "targetUsers": [
        ...
        "coreFeatures": [
            {
                ...
                "description": "Pairs each new student with a senior international peer based on country, language, major, and shared interests.",
            },
            {
                "name": "Weekly Pods",
                "description": "Small 4-6 person groups that meet weekly around themed prompts (food, homesickness, exam stress).",
            },
            {
                "name": "Soft Journal",
                "description": "Private space for short daily reflections with gentle, non-clinical prompts.",
            },
            {
                "name": "Campus Pulse",
            ...
            {
                "name": "Crisis Bridge",
                "description": "One tap to access verified human counselors and the campus international office.",
            },
        ],
        "userJourney": [
...
def build_claude_prompt(idea: str) -> str:
    return f"""You are Claude Code, a senior full-stack engineer.
...
def build_brand_campaign(idea: str) -> dict:
    return {
        "brandName": PRODUCT_NAME,
        "tagline": "A softer landing in your new country.",
        "mission": (
            "BridgeMate exists so that no international student has to face their first semester abroad alone. "
            "We pair newcomers with senior peers who have been exactly where they are."
        ),
        "heroHeadline": "Your first semester abroad, less lonely.",
        "heroSubheadline": (
            "BridgeMate matches you with a senior international student at your university and a small weekly pod, "
            "so the hardest part of moving doesn't have to be the loneliest."
        ),
        ...
        "socialCaption": (
            "Moving abroad is the hardest soft thing you'll ever do. BridgeMate pairs you with a senior international "
            "student who's been exactly where you are. First semester, but warmer. #BridgeMate #StudyAbroad"
        ),
        "launchAnnouncement": (
            "Today we're opening BridgeMate to international students at 12 U.S. universities. "
            "If you remember what your first month abroad felt like, you know why we built this. "
            "Sign up, get matched in 48 hours, and join a Weekly Pod that meets every Sunday."
        ),
        "voiceAndTone": [
            "Warm, not clinical.",
...
def build_visual_prompts(idea: str) -> dict:
    return {
        "heroImage": (
            "Editorial photograph of a young international student standing on a quiet university quad at golden hour, "
            "soft sunlight, slight motion blur of other students walking past, warm autumn tones, "
            "shallow depth of field, hopeful and slightly bittersweet mood, cinematic 35mm look, "
            "no logos, no text, 16:9 aspect ratio."
        ),
        "campaignPoster": (
            "Minimalist campaign poster, soft gradient background blending warm peach and deep navy, "
            "centered serif headline 'A softer landing in your new country.', small BridgeMate wordmark in lower right, "
            "an illustrated paper-airplane icon trailing a thin line across the poster, "
            "generous negative space, premium print feel, 2:3 portrait ratio."
        ),
        "uiMoodboard": (
            "UI moodboard for a calm, warm web app for international students. Color palette: deep navy background, "
            "warm cream text, accent gradients of peach to violet. Typography: large humanist serif for headlines, "
            "clean geometric sans for body. Components: rounded-2xl cards with soft inner glow, soft drop shadows, "
            "no harsh borders, gentle micro-illustrations of birds, paper boats, and lanterns. Mood: hopeful, "
            "quiet, slightly homesick. 4 panels in a 2x2 grid."
        ),
        "architectureDiagram": (
            "Clean technical architecture diagram for a web app named BridgeMate. Three vertical lanes labeled "
            "'Client (Next.js)', 'API (Flask)', and 'Data (Postgres)'. Show arrows from Client through API to Data, "
            "and side modules for Matching Service, Pod Scheduler, Soft Journal Store, and Crisis Bridge Resource. "
            "Use a dark navy background, soft cyan and violet accent lines, minimalist icons, "
            "clear monospaced labels, 16:9 landscape, presentation-ready."
        ),
    }


def build_evaluation(idea: str) -> dict:
    return {
        "scores": {
            "productMarketFit": 86,
            "userEmpathy": 92,
        ...
        "judgeNotes": (
            "Overall, this concept is unusually thoughtful for a v1. The biggest risk is not technical, it is ethical: "
            "shipping a peer-support product without real moderation and crisis partners. Address that before the "
            "campaign goes live."
        ),
    }
...
def build_workflow() -> list:
    return [
        ...
        },
        {
            ...
            "model": "ChatGPT",
            "description": "Compiles a precise Claude Code build brief with constraints and acceptance criteria.",
        },
        {
            "stage": "Code",
            "owner": "Coding Agent",
...
def build_full_package(idea: str) -> dict:
    return {
        "rawIdea": idea,
        "research": build_research(idea),
        "blueprint": build_blueprint(idea),
        "claudeBuildPrompt": build_claude_prompt(idea),
        "brandCampaign": build_brand_campaign(idea),
        "visualPrompts": build_visual_prompts(idea),
        "evaluation": build_evaluation(idea),
        "workflow": build_workflow(),
    }


def build_sample_package() -> dict:
    return build_full_package(SAMPLE_IDEA)
